...
pub fn satisfies(
    ...
    requirements.iter().all(|req| satisfies_one(req, capabilities))
}

fn satisfies_one(req: &CapabilityRequirement, caps: &WorkerCapabilities) -> bool {
    match req {
        CapabilityRequirement::Tag { value } => caps.has_tag(value),
        CapabilityRequirement::Scalar { name, value } => caps
            .scalar(name)
            .map(|cap| cap >= *value)
            .unwrap_or(false),
    }
...
mod tests {
    ...
    fn tag(value: &str) -> CapabilityRequirement {
        CapabilityRequirement::Tag { value: value.to_string() }
    }

    fn scalar(name: &str, value: f64) -> CapabilityRequirement {
        CapabilityRequirement::Scalar { name: name.to_string(), value }
    }
