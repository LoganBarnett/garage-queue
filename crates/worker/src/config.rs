...
impl Config {
    pub fn from_cli_and_file(cli: CliRaw) -> Result<Self, ConfigError> {
        ...
        let server_url =
            worker.server_url.unwrap_or_else(|| "http://127.0.0.1:9090".to_string());
        let poll_interval_ms = worker.poll_interval_ms.unwrap_or(1000);

        let control = file.control.unwrap_or(ControlSectionRaw { host: None, port: None });
        let control_host = control.host.unwrap_or_else(|| "127.0.0.1".to_string());
        let control_port = control.port.unwrap_or(9091);
        let control_bind =
            format!("{control_host}:{control_port}").parse().map_err(|source| {
